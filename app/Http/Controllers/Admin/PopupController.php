...
use App\Models\PopupModel as MainModel;
...
class PopupController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->controllerName = "{$this->prefix}/popup";
        $this->pathViewController = "{$this->prefix}.pages.pop-up";
    ...
    public function save(Request $request)
    {
        ...
        if (empty($error)) {
            ...
            if ($request->hasFile('pop_image')) {
                $file  = $request->file('pop_image');
                $originalName = $file->getClientOriginalName();
                $originalName = pathinfo($originalName, PATHINFO_FILENAME);
                $extension = $file->clientExtension();
                $name = $originalName . "-" . \Str::random(10) . "." . $extension;
                $file->storeAs('popup', $name, 'rrt_storage');
                $params['pop_image'] = $name;
            }
            else{
                unset($params['pop_image']);
            }
    ...
    function list(Request $request)
    {
        ...
        foreach ($data as &$item) {
            $item['pop_image'] = $item['pop_image']
                ? url('public/uploads/popup/' . $item['pop_image'])
