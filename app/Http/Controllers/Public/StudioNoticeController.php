...
#Helper
class StudioNoticeController extends Controller
{
    private $prefix;
    ...
    public function __construct()
    {
        ...
        $this->controllerName = "public/{$this->prefix}/notice";
    ...
    public function maskAsRead(Request $request){
        ...
        $noti->status = $this->model::IS_READ;
        $noti->save();
        return [
            'success' => true,
            'message' => 'Item mask as read',
            'status' => $this->model::IS_READ
        ];
    }
    public function maskAsReadAll(){
        $user_id  = rrt_get_user_login('id');
        $this->model->where('user_id',$user_id)->update(['status'=>$this->model::IS_READ]);
        return [
            'success' => true,
            'message' => 'Mask as read all items',
        ];
    }
}
