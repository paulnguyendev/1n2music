...
{
    protected $table = 'rrt_ai_packages';
    protected $fillable = ['id','name','ai_id','created_at','updated_at'];
        ...
        if ($options['task'] == 'admin') {
            ...
            if (isset($params['is_map'])) {
                $result = $result->map(function ($item) use ($params) {
                    ...
                    $item->name = '<a href="'.rrt_route('admin/aiPackageRole/editRoles',['id'=>$id]).'">'.$item->name??"(No name)".'</a>';
                    $item->ai_service = $item->aiService ?? "";
    ...
    {
        return $this->belongsToMany(Role::class, 'rrt_ai_package_roles', 'package_id', 'role_id')
