...
class FaqModel extends Model
{
    ...
    protected $fillable = ['id', 'name','content','category_id','order_number', 'created_at'];
        ...
        if ($options['task'] == 'admin') {
            ...
            if (isset($params['is_map'])) {
                $result = $result->map(function ($item) use ($params) {
                    ...
                    $item->route_update = rrt_route($controllerName . "/update", ['id' => $id,'category' => $category]);
                    $item->route_edit = rrt_route($controllerName . "/form", ['id' => $id,'category' => $category]);
                    $item->route_remove = rrt_route($controllerName . "/delete", ['id' => $id,'category' => $category]);
                    return $item;
                });
            }
