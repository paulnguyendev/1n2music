...
class GenresModel extends Model
{
    ...
    protected $fillable = ['id', 'name', 'thumbnail','parent_id', 'order_number'];
    protected $checkEmail = ['id', 'user_id', 'first_name', 'middle_name', 'last_name', 'fullname', 'email'];
    use HasFactory, SoftDeletes;
    public function listItems($params = "", $options = "")
    {
        $result = null;
        $query = $this->select($this->fillable)->whereNull('parent_id');
        if ($options['task'] == 'list') {
            if (isset($params['start']) && isset($params['length'])) {
                $result = $query->orderBy('order_number', 'asc')->skip($params['start'])->take($params['length'])->get();
            } else {
        ...
        if ($options['task'] == 'admin') {
            ...
            if (isset($params['is_map'])) {
                $result = $result->map(function ($item) use ($params, $result) {
                    $id = $item->id;
                    $item->plan_info = "123";
                    $status = $item->status ?? "";
                    if ($status == 'active') {
                    ...
                    $item->route_update_order_number = rrt_route($controllerName . "/updateOrderNumber", ['id' => $id]);
                    return $item;
                });
            }
    ...
    {
        return $this->hasMany(GenresModel::class, 'parent_id', 'id')->orderBy('order_number', 'asc');
    }
