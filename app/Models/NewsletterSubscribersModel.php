...
{
    use SoftDeletes;
    protected $table = 'rrt_newsletter_subscribers';
        ...
        $query = $this->select($this->fillable);
            ...
            if (isset($params['is_map'])) {
                $result = $result->map(function ($item) use ($params) {
                    ...
                    // $item->route_update = rrt_route($controllerName . "/subscriberUpdate", ['id' => $id]);
                    $item->route_edit = rrt_route($controllerName . "/form", ['id' => $id]);
                    $item->route_remove = rrt_route($controllerName . "/subscriberDelete", ['id' => $id]);
                    return $item;
                });
            }
