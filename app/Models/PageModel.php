...
{
    protected $table = 'rrt_pages';
    protected $primaryKey = 'id';
    public $timestamps = false;
    const CREATED_AT = 'created_at';
    const UPDATED_AT = 'updated_at';
    protected $fieldSearchAccepted = ['email', 'phone', 'fullname'];
    protected $crudNotAccepted = ['_token', 'confirm_password', 'is_agree', 'cycle', 'plan_order', 'page', 'subscription_order', 'data_attributes','translations'];
    protected $fillable = ['id', 'name', 'content','slug','description','created_at','updated_at','image','type'];
        ...
        if ($options['task'] == 'admin') {
            ...
            if (isset($params['is_map'])) {
                $result = $result->map(function ($item) use ($params, $type) {
                    ...
                    $item->route_edit = rrt_route($controllerName . "/form", ['id' => $id,'type' => $type]);
    ...
    {
        return $this->hasMany(PageTranslationModel::class, 'page_id');
    }
