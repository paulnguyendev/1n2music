...
class SearchModel extends Model
{
    ...
    protected $fillable = ['id', 'key_word','ip', 'count', 'created_at'];
    protected $checkEmail = ['id', 'user_id', 'first_name', 'middle_name', 'last_name', 'fullname', 'email'];
    use HasFactory, SoftDeletes;

    public function addKeyWord($key_word)
    {
        $check =   $this->where('key_word', $key_word)->first();
        if ($check) {
            $count =  $check->count + 1;
            $check->update(['count' => $count]);
        } else {
            $this->create(['key_word' => $key_word, 'count' => 1]);
        }
    }
    ...
    public function getItem($params = [], $options = [])
    {
        ...
        $query = $this->select($this->fillable)->orderBy('updated_at','desc');
