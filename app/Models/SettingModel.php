...
class SettingModel extends Model
{
    ...
    public function saveItem($params = [])
    {
        ...
        unset($paramsInsert['data_attributes']);
        foreach ($paramsInsert as $key => $value) {
            $meta_value =    $this->where('meta_key', $key)->first();
            if ($meta_value) {
                $result =     $meta_value->update(['meta_value' => $value]);
            } else {
                $result =   self::create([
                    'meta_key' => $key,
                    'meta_value' => $value
    ...
    public function getOrCreateCommissions()
    {
        $commissionKeys = [
            'commission_seller' => 0,
            'commission_publishing' => 0,
            'commission_distribute'=>0
        ];
        $commissions = [];
        foreach ($commissionKeys as $key => $defaultValue) {
            $commission = $this->where('meta_key', $key)->first();
            ...
            $commissions[$key] = $commission->meta_value;
        }
