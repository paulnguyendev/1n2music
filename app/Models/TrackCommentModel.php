...
class TrackCommentModel extends Model
{
    protected $table = 'rrt_comment_track';
    ...
    protected $fillable = ['id', 'user_id', 'track_id', 'content', 'parent_id', 'created_at'];
    ...
    public function listItems($params = "", $options = "")
    {
        ...
        if ($options['task'] == 'count') {
            if ($params['arr_track']) {
                $result =    $this->whereIn('track_id', $params['arr_track'])->count();
            }
        }
        if ($options['task'] == 'comment-dashboard') {
            if ($params['arr_track']) {
                $result =    $this->whereIn('track_id', $params['arr_track'])->orderBy('id', 'desc')->take(3)->get();
            }
        }
        if ($options['task'] == 'admin') {
            ...
            if (isset($params['is_map'])) {
                $result = $result->map(function ($item) use ($params) {
                    ...
                    $item->route_update = route($controllerName . "/update", ['id' => $id]);
                    $item->route_edit = route($controllerName . "/form", ['id' => $id]);
                    $item->route_remove = route($controllerName . "/delete", ['id' => $id]);
                    return $item;
                });
            }
        ...
        if ($options['task'] == 'ajax') {
            ...
            $comment =    $this->where('id', $params['id'])->first();

            $query =  $this->where('track_id', $comment->track_id)
                ->where('parent_id', $comment->parent_id)
                ->where('id', '<', $comment->id)->with('user')->limit($this->limit);
            $result = $query->orderBy('id', 'desc')->get();
        }
        return $result;
    }
    ...
    public function getListToTrack($params)
    {
        $this->limit = $params['limit'] ?? 5;
        return  $this->where('track_id', $params['track_id'])
            ->where('parent_id', 0)->with('comment')
            ->with('user')
    ...
    public function comment()
    {
        ...
        return $this->hasMany(TrackCommentModel::class, 'parent_id')->with('comment')->with('user')->orderBy('created_at', 'desc')->limit(2);
    }
    ...
    {
        return $this->belongsTo(TrackModel::class, 'track_id', 'id')->with('file');
    }
}
