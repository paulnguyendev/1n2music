...
class TrackFavouritesModel extends Model
{
    protected $table = 'rrt_track_favourites';
    ...
    protected $fillable = ['id', 'track_id', 'user_id','created_at'];
    ...
    public function listItems($params = "", $options = "")
    {
        ...
        if($options['task'] == 'count') {
            $result = $query->where('track_id', $params['track_id'])->count();
        }
        return $result;
    }
    public function getItem($params = [], $options = [])
    {
        ...
        if ($options['task'] == 'check') {
            $result = $query->where('track_id', $params['track_id'])->where('user_id', $params['user_id'])->first();
        }
        return $result;
    }
    ...
    {
        return $this->belongsTo(TrackModel::class,'track_id','id')->with('file')->with('listContracts')->with('user');
    }
