@extends('admin.main')
@section('page_title', __('member.seller_members'))
@section('title', __('member.seller_members'))
@section('css')
    <link href="https://cdn.jsdelivr.net/gh/gitbrent/bootstrap4-toggle@3.6.1/css/bootstrap4-toggle.min.css" rel="stylesheet">

@section('buttons')
    <!-- <a data-url="{{ rrt_route('admin/tools/packageUsage', ['account_type' => 'free-user']) }}"
            data-toggle="modal"
    ...
    <a href="{{ rrt_route($controllerName . '/form', ['account_type' => 'user']) }}"
        class="btn btn-primary">{{ __('member.create_a_new_account') }}</a>
@endsection
            ...
            <div class="card">
                <div class="card-body">
                    <table class="table table-xlg datatable-ajax"
                        data-source="{{ rrt_route($controllerName . '/list', ['account_type' => 'free-user']) }}"
                        ...
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('member.Username') }}</th>
                                <th>{{ __('member.Fullname') }}</th>
                                <th>{{ __('member.Email') }}</th>
                                <th>{{ __('member.Phone') }}</th>
                                <th>{{ __('member.Home_Page') }}</th>
                                <th>{{ __('member.Join_Type') }}</th>
                                {{-- <th>Tax Type</th> --}}
                                <th width="50">{{ __('member.Status') }}</th>
...
@push('script')
    ...
    <script>
        ...
        WBDatatables.addDownloadButton(`{{rrt_route($controllerName . "/export",['type' => 'basic'])}}`, 'Export Users');
        ...
        $(document).ready(function() {
            ...
            $('#save-changes').on('click', function(e) {
                ...
                if (!aiCount || isNaN(aiCount) || aiCount == 0) {
                    $('#ai-count').after(
                        '<span class="error-message text-danger">Please enter a valid number</span>');
                    hasError = true;
                }
                if (!hasError) {
                    $.ajax({
