                    ...
                    @foreach ($plans as $key => $item)
                        ...
                        <div class="pricing-item {{ $isFree == '1' ? 'is_free' : '' }}"
                            ...
                            <div class="pricing-top">
                                ...
                                @if ($slug === 'publishing')
                                    <div class="coming-soon" style="text-align: center; font-size: 18px; color: #FF0000; min-height: 75px">
                                        {{ __('Coming Soon') }}
                                    </div>
                                @else
...
@push('srcipt')
    <script>
        function setEqualHeight() {
            var elements = document.querySelectorAll('.pricing-item');

            var maxHeight = 0;
            var priceMaxHeight = 0;
            elements.forEach(function(container) {
                var contentPricing = container.querySelector('.pricing-item-feature');
                var pricingTop =  container.querySelector('.pricing-item-desc');
                if (contentPricing) {
                    var height = contentPricing.offsetHeight;
                    if (height > maxHeight) {
                        maxHeight = height;
                    }
                }
                if(pricingTop){
                    var pricingHeight = pricingTop.offsetHeight;
                    if(pricingHeight > priceMaxHeight){
                        priceMaxHeight = pricingHeight;
                    }
                }
            });

            elements.forEach(function(container) {
                var pricing = container.querySelector('.pricing-item-desc');
                var contentPricing = container.querySelector('.pricing-item-feature');
                if (pricing) {
                    pricing.style.minHeight = priceMaxHeight + 'px';
                }
                if (contentPricing) {
                    contentPricing.style.height = maxHeight + 'px';
                }
            });
        }
        setEqualHeight()
        ...
        $('#submitPlans').click(function () {
            ...
            $.ajax({
                ...
                url: '{{ rrt_route($controllerName . '/postStartSelling') }}',
                type: 'POST',
                data: data,
                beforeSend: function() {
                    showLoading();
