    ...
    <div class="section-cart section-padding">
        <div class="container">
            ...
            <div class="text-left" style="margin-bottom:0px;">
                ...
                <form id="checkout-form" action="{{ rrt_route('public/checkout/postCheckout') }}" method="POST">
                    @csrf
                    @foreach ($pendingPlans as $orderPlan)
                    <input type="hidden" name="plan_ids[]" value="{{ $orderPlan->id ?? '' }}">
                    @endforeach
                    ...
                    <div class="text-center">
                        @php 
                            $cookieCancel = Cookie::get('first_payment_reminder');
                        @endphp
                        @if($cookieCancel && $cookieCancel == "true")
                        <a href="{{ rrt_route('public/checkout/cancel',['user_id' => $user_id]) }}" 
                            class="btn text-center" 
                            style="background: gray; color: white;"
                            onclick="confirmCancelOrder(event, this)">
                            {{ __('Cancel Order') }}
                        </a>
                        @endif
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if (response.status === 200) {
                            showNotify('success', 'Success', response.msg); // Sử dụng showNotify để hiển thị thông báo thành công
                            if (response.redirect) {
                                window.location.href = response.redirect; // Chuyển hướng nếu có URL trong phản hồi
                            }
                        } else {
                            showNotify('error', 'Error', response.msg || 'Order cancellation failed.'); // Sử dụng showNotify để hiển thị thông báo lỗi
                        }
                    },
                    error: function () {
                        showNotify('error', 'Error', 'An error occurred during order cancellation.'); // Thay toast bằng showNotify
                    },
                    complete: function () {
                        hideLoading();
                    }
