    ...
    <div class="section-checkout section-padding">
        <div class="container">
                ...
                <div class="col-lg-6">
                    ...
                    <div class="customer-info mt-30">
                        <div class="form-group">
                            ...
                            <input type="email" name="fullname" class="form-control" value="{{rrt_get_user_login('fullname')}}" readonly required>
                        </div>
                        <div class="form-group">
                            <label for="email">{{__('Phone')}}</label>
                            <input type="email" name="email" class="form-control" value="{{rrt_get_user_login('phone')}}" readonly required>
                        </div>
                        <div class="form-group">
                            <label for="email">{{__('Email Address')}}</label>
                            <input type="email" name="email" class="form-control" value="{{rrt_get_user_login('email')}}" readonly required>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="checkout-title">
                        <i class="fa fa-shopping-cart fa-lg"></i> {{__('Order Information')}}
                    </div>
                    <div class="checkout-items">
                        <table class="table-checkout-item">
                            <thead>
                            <tr>
                                <th style="width:50%">{{$subscription->name??''}}</th>
                                <th>{{__('Price')}}</th>
                            </tr>
                            <tbody>
                            <tr>
                                <td>
                                    <p>
                                        <strong>{{ $subscription->heading ?? '' }}</strong>
                                    </p>
                                ...
                                <td class="text-center">
                                    {{ rrt_show_price($subscription->price ?? 0) }}
                                </td>
                            </tr>
                            </tbody>
                            <tfoot>
                            <tr>
                                ...
                                <td class="text-center"><strong
                                        class="track-meta-price">{{ rrt_show_price($subscription->price ?? 0) }}</strong></td>
                            </tr>
                            </tfoot>
                            </thead>
                        </table>
            ...
            <div class="text-left">
                ...
                <form id="checkout-form" action="{{rrt_route('public/join/payment/create')}}" method="POST">
                    @csrf
                    <input type="hidden" name="user_id" value="{{rrt_get_user_login('id')}}">
                    <input type="hidden" name="subscription_id" value="{{$subscription->id??''}}">
                    <input type="hidden" name="price" value="{{$subscription->price ?? 0}}">
...
@push('srcipt')
    ...
    <script>
        ...
        formCheckout.submit(function(e) {
            ...
            if (!data.user_id || !data.subscription_id || !data.email || !data.payment_method || !data.price) {
